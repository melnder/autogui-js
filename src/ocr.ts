import { screenshot, ScreenshotImage, type ScreenshotRegion } from "./system.js";
...
const DIGIT_TEMPLATES: Record<string, string[]> = {
    "0": [
        "01111110",
        "11111111",
        "11000011",
...
const EXTRA_DIGIT_TEMPLATES: Record<string, string[][]> = {
    "1": [[
        "......######",
        "..##########",
        "############",
...
const NORMALIZED_TEMPLATES = Object.fromEntries(
    Object.entries(DIGIT_TEMPLATES).map(([digit, rows]) => [
        digit,
        [
...
function isTemplateTextPixel(value: string): boolean {
    return value === "1" || value === "#";
}

function normalizeTemplate(rows: string[]): Uint8Array {
...
function normalizeGlyph(mask: Uint8Array, imageWidth: number, bounds: Bounds): Uint8Array {
    ...
    for (let y = 0; y < TEMPLATE_HEIGHT; y += 1) {
        ...
        const sourceYEnd = bounds.y + Math.max(
            Math.floor((y + 1) * bounds.height / TEMPLATE_HEIGHT),
            Math.floor(y * bounds.height / TEMPLATE_HEIGHT) + 1,
        );

        for (let x = 0; x < TEMPLATE_WIDTH; x += 1) {
            const sourceXStart = bounds.x + Math.floor(x * bounds.width / TEMPLATE_WIDTH);
            const sourceXEnd = bounds.x + Math.max(
                Math.floor((x + 1) * bounds.width / TEMPLATE_WIDTH),
                Math.floor(x * bounds.width / TEMPLATE_WIDTH) + 1,
            );
...
function templateConfidence(glyph: Uint8Array, template: Uint8Array): number {
    let matches = 0;
...
function recognizeDigit(glyph: Uint8Array): { text: string, confidence: number } {
    let bestText = "";
...
function averageConfidence(chars: DigitOcrChar[]): number {
    if (chars.length === 0) {
        return 0;
    }
...
export function readDigits(image: ScreenshotImage, options: DigitOcrOptions = {}): DigitOcrResult {
...
export function readDigitsOnScreen(region: ScreenshotRegion, options?: DigitOcrOptions): DigitOcrResult;
export function readDigitsOnScreen(
    x: number,
    y: number,
    width: number,
    height: number,
...
export function readDigitsOnScreen(
    regionOrX: ScreenshotRegion | number,
    yOrOptions: number | DigitOcrOptions = {},
    width?: number,
    height?: number,
    options: DigitOcrOptions = {},
): DigitOcrResult {
    if (typeof regionOrX === "number") {
        ...
        return readDigits(screenshot(regionOrX, yOrOptions, width as number, height as number), options);
    }

    return readDigits(screenshot(regionOrX), yOrOptions as DigitOcrOptions);
}
